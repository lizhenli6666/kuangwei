...
function animate(div, obj) {
  ...
  div.timer = setInterval(function () {
    ...
    for (var key in obj) {
      ...
      if (key == 'opacity') {
        var speed = (target - parseFloat(getStyle(div)[key])) * 100 / 8; // console.log(speed,'speed1')

        speed = speed > 0 ? Math.ceil(speed) : Math.floor(speed); //console.log(speed,'speed2')

        var op = parseFloat(getStyle(div)[key]) + speed / 100; // console.log(op)
